"""Entry point: run Evaluation 1 against every configured LLM provider."""
...
def main():
    ...
    with open("eval1_results.jsonl", "w", encoding="utf-8") as f:
        for r in results:
    ...
    pd.DataFrame(rows).to_csv("eval1_results.csv", index=False)
    if rows:
